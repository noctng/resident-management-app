...
exports.getContractById = async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({ message: 'Lỗi máy chủ khi tải chi tiết hợp đồng' });
    }
};
    ...
    try {
        ...
        const newContract = await prisma.$transaction(async (tx) => {
            ...
            if (payment_schedule && Array.isArray(payment_schedule)) {
                const payments = payment_schedule.map((p, index) => ({
                ...
                await tx.contract_payments.createMany({
                    data: payments,
                });
            }
...
exports.updateContract = async (req, res) => {
    try {
        const { id } = req.params;
        const { status, signed_date, handover_date } = req.body;

        // Fetch old contract for change tracking
        const oldContract = await prisma.contracts.findUnique({
            where: { id },
        });
        ...
        const action = status === 'CANCELLED' ? 'CANCEL_CONTRACT' : 'UPDATE';
        await logActivity(
            req.user,
            action,
            'CONTRACT',
