...
const repo = require('../repositories/executiveAnalyticsRepository');

const buildAptWhere = (phase) => (phase && phase !== 'ALL' ? { phase_code: phase } : {});

// ============ OVERVIEW ============
const getOverview = async (phase, year) => {
  const aptWhere = buildAptWhere(phase);
  const [contracts, allPayments, constructionRegs, claims, totalApartments] = await Promise.all([
    repo.getContracts(aptWhere),
    repo.getOverduePayments(aptWhere),
    repo.getConstructionRegs(aptWhere),
    repo.getWarrantyClaims(aptWhere),
    repo.countApartments(aptWhere),
  ]);
...
// ============ SALES FUNNEL ============
const getSalesFunnel = async (phase) => {
  const aptWhere = buildAptWhere(phase);
  const [leadsCount, bookingsCount, depositsCount, contractsCount, transfersCount, phaseRevenue, commissions] =
    await Promise.all([
      repo.countLeads(),
      repo.countBookings(aptWhere),
      repo.countDeposits(aptWhere),
      repo.countContracts(aptWhere),
      repo.countTransfers(aptWhere),
      Promise.all(['CANTATA', 'TESLA', 'DA_VINCI'].map(async (p) => {
        const pContracts = await repo.getPhaseContracts(p);
        const val = pContracts.reduce((sum, c) => sum + Number(c.total_value || 0), 0);
        return { phase: p, count: pContracts.length, totalValue: val };
      })),
      repo.getCommissions(aptWhere),
    ]);
  ...
  return {
    ...
    phaseRevenue,
    commissions: { totalCommission, paidCommission, remainingCommission: totalCommission - paidCommission },
  };
};

// ============ FINANCIAL AGING ============
const getFinancialAging = async (phase) => {
  const aptWhere = buildAptWhere(phase);
  const now = new Date();
  const pendingPayments = await repo.getPendingPayments(aptWhere);
  ...
  let forecast30d = 0, forecast60d = 0, forecast90d = 0;
  ...
  for (const p of pendingPayments) {
    ...
    if (dueDate >= now) {
      ...
      if (diffDays <= 30) { aging.d0_30.amount += amt; aging.d0_30.count += 1; aging.d0_30.payments.push(item); }
      else if (diffDays <= 60) { aging.d31_60.amount += amt; aging.d31_60.count += 1; aging.d31_60.payments.push(item); }
      else if (diffDays <= 90) { aging.d61_90.amount += amt; aging.d61_90.count += 1; aging.d61_90.payments.push(item); }
      else { aging.d90plus.amount += amt; aging.d90plus.count += 1; aging.d90plus.payments.push(item); }
    }
  }
...
// ============ OPERATIONS SLA ============
const getOperationsSla = async (phase) => {
  const aptWhere = buildAptWhere(phase);
  const [claims, contractors, fitouts] = await Promise.all([
    repo.getWarrantyClaimsFull(aptWhere),
    repo.getContractors(aptWhere),
    repo.getFitouts(aptWhere),
  ]);
  ...
  const pareto = Object.keys(categoryMap)
    .map((cat) => ({
      category: cat,
...
// ============ COMMUNITY OCCUPANCY ============
const getCommunityOccupancy = async (phase) => {
  const aptWhere = buildAptWhere(phase);
  const apartments = await repo.getApartmentsFull(aptWhere);
  ...
  const totalCars = apartments.reduce((sum, a) => sum + a.vehicles.filter((v) => v.vehicle_type === 'CAR').length, 0);
  const totalBikes = apartments.reduce((sum, a) => sum + a.vehicles.filter((v) => v.vehicle_type === 'MOTORBIKE').length, 0);
  ...
  return {
    community: {
      ...
      totalResidents,
      vehicles: { cars: totalCars, motorbikes: totalBikes, total: totalCars + totalBikes },
    },
  };
};

// ============ EXPORT EXECUTIVE REPORT ============
const exportExecutiveReport = async (res) => {
  ...
  const contracts = await repo.getContractsExport();
  for (const c of contracts) {
    const paid = c.contract_payments
      .filter((p) => p.status === 'PAID')
      .reduce((sum, p) => sum + Number(p.paid_amount || p.amount || 0), 0);
    ws1.addRow({
      code: c.contract_code, apartment: c.apartments?.code, customer: c.customers?.name,
      total_value: Number(c.total_value || 0), paid, status: c.status,
    });
  }
  ...
  const overdue = await repo.getOverdueExport();
  for (const p of overdue) {
    ws2.addRow({
      contract: p.contracts?.contract_code, apartment: p.contracts?.apartments?.code,
      customer: p.contracts?.customers?.name, installment: p.installment_number,
      due_date: new Date(p.due_date).toLocaleDateString('vi-VN'), amount: Number(p.amount || 0),
    });
  }
  ...
  const claims = await repo.getWarrantyClaimsExport();
  for (const cl of claims) {
    ws3.addRow({
      claim_code: cl.claim_code, apartment: cl.apartments?.code, category: cl.category,
      severity: cl.severity, contractor: cl.contractors?.name || 'Chưa gán', status: cl.status,
    });
  }
