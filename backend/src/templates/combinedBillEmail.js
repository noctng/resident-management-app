/**
 * Generate HTML email template for combined billing (Electricity, Water, Management Fees)
 */
function generateCombinedBillEmail(data) {
    const {
        residentName,
        apartmentCode,
        month,
    ...
    // Calculate management fee rows
    let managementFeeRows = '';
    if (management_fee_breakdown) {
        const fees = [
            { label: 'Phí quản lý', value: management_fee_breakdown.management_fee },
            { label: 'Phí Internet', value: management_fee_breakdown.internet_fee },
            { label: 'Phí truyền hình cáp', value: management_fee_breakdown.cable_tv_fee },
            { label: 'Phí gửi xe ô tô', value: management_fee_breakdown.parking_car_fee },
            { label: 'Phí gửi xe máy', value: management_fee_breakdown.parking_motorbike_fee },
            { label: 'Phí an ninh', value: management_fee_breakdown.security_fee },
            { label: 'Phí vệ sinh', value: management_fee_breakdown.cleaning_fee },
        ];
        ...
        fees.forEach((fee) => {
            if (fee.value > 0) {
                managementFeeRows += `
                <tr>
                    <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${fee.label}</td>
                    <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatCurrency(fee.value)}</td>
                </tr>`;
            }
        });
    }
...
<div style="max-width: 650px; margin: 20px auto; padding: 4px; border: 1px solid #e0e6ed; background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.05);">
    ...
    <p style="margin: 2px 0; font-size: 14px; line-height: 1.15;">Kính gửi Quý Cư Dân <strong style="color: #d32f2f;">${residentName || 'Quý Cư Dân'}</strong>,</p>
    
    <p style="margin: 2px 0 6px; font-size: 14px; line-height: 1.15;">Ban Quản lý xin thông báo chi phí tổng hợp của căn hộ <strong>${apartmentCode}</strong> trong kỳ <strong>${month}/${year}</strong> như sau:</p>
    ...
    <table style="width: 100%; border-collapse: collapse; margin: 6px 0; font-size: 13px; border: 1px solid #d9e2ec;">
        ...
        <tr style="background-color: #e8f0fe; color: #1a365d;">
            ...
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(water_old_reading || 0)} m³</td>
        </tr>
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Chỉ số nước mới</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(water_new_reading || 0)} m³</td>
        </tr>
        <tr style="background-color: #e8f0fe; color: #1a365d;">
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Chỉ số nước tiêu thụ</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(water_usage || 0)} m³</td>
        </tr>
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Tiền nước</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatCurrency(water_cost || 0)}</td>
        </tr>
        ...
        <tr style="background-color: #e8f0fe; color: #1a365d;">
            ...
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(electricity_old_reading || 0)} kWh</td>
        </tr>
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Chỉ số điện mới</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(electricity_new_reading || 0)} kWh</td>
        </tr>
        <tr style="background-color: #e8f0fe; color: #1a365d;">
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Chỉ số điện tiêu thụ</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatNumber(electricity_usage || 0)} kWh</td>
        </tr>
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Tiền điện</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatCurrency(electricity_cost || 0)}</td>
        </tr>
        ...
        ${managementFeeRows || '<tr><td colspan="2" style="padding: 3px 4px; font-style: italic; text-align: center;">Không có phí phát sinh</td></tr>'}
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Tổng phí quản lý</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${formatCurrency(management_fee_cost || 0)}</td>
        </tr>
        ...
        <tr style="background-color: #fff9f9;">
            ...
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; color: #d32f2f; font-weight: bold; line-height: 1.15;">${formatCurrency(grand_total || 0)}</td>
        </tr>
        <tr>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Hạn nộp</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; line-height: 1.15;">${dueDate}</td>
        </tr>
        <tr style="background-color: #e8f0fe; color: #1a365d;">
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-weight: bold; line-height: 1.15;">Nội dung CK</td>
            <td style="padding: 3px 4px; border: 1px solid #d9e2ec; font-family: monospace; line-height: 1.15;">${transferContent}</td>
        </tr>
    </table>
    ...
    <div style="text-align: center; margin: 6px auto; padding: 4px; background-color: #f1f5f9; border-radius: 6px; width: fit-content; max-width: 100%;">
        ...
        <div style="display: flex; justify-content: center;">
            <img src="${qrCodeUrl}" width="180" alt="QR Code" style="display: block; margin: 0 auto; border: 1px solid #cbd5e1; padding: 4px; background-color: white; border-radius: 4px;">
        </div>
    </div>
    `
            : ''
    ...
    <div style="margin: 6px 0; padding: 4px; background-color: #fefefe; border: 1px dashed #94a3b8; border-radius: 6px; font-size: 13px; line-height: 1.15;">
        ...
        <ul style="margin: 2px 0; padding-left: 18px; line-height: 1.15;">
            <li style="margin: 1px 0; line-height: 1.15;">Tài khoản: <strong>${bankAccount.accountName || 'BAN QUAN LY'}</strong></li>
            <li style="margin: 1px 0; line-height: 1.15;">Số tài khoản: <strong>${bankAccount.accountNumber || '123456789'}</strong></li>
            <li style="margin: 1px 0; line-height: 1.15;">Ngân hàng: <strong>${bankAccount.bankName || 'Ngan Hang'}</strong></li>
            <li style="margin: 1px 0; line-height: 1.15;">Nội dung CK: <strong>${transferContent}</strong></li>
        </ul>
