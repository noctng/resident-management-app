...
interface ImportResidentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export const ImportResidentModal: React.FC<ImportResidentModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const handleDownloadTemplate = () => {
    window.open('/api/residents/import/template', '_blank');
  };

  const handleUpload = async () => {
    if (!file) {
      toast.warning('Vui lòng chọn file Excel danh sách cư dân để tải lên!');
      return;
    }
    ...
    try {
      ...
      formData.append('autoCreateAccount', String(autoCreateAccount));
      ...
      const res = await api.post<{
        ...
        errorCount: number;
        errors?: Array<{ row: number; name?: string; error: string }>;
      }>('/residents/import', formData);

      if (res) {
        setResult(res);
        toast.success(res.message || 'Đã import danh sách cư dân thành công!');
        onSuccess();
      }
    } catch (err: any) {
      console.error('Import residents error:', err);
      toast.error(err.response?.data?.message || err.message || 'Lỗi khi import danh sách cư dân');
    } finally {
      setImporting(false);
    }
  };
      ...
      <div className="bg-surface rounded-2xl shadow-xl border border-brand-border w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-up">
        ...
        <div className="px-6 py-4 bg-sidebar-bg text-white flex items-center justify-between border-b border-sidebar-line shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-accent-soft/20 text-accent-soft flex items-center justify-center border border-accent/30">
              <UsersIcon className="w-6 h-6 text-accent" />
            </div>
            <div>
              <h3 id="import-resident-modal-title" className="font-bold text-base text-white flex items-center gap-2">
                <span>Import Danh Sách Cư Dân</span>
              </h3>
              <p className="text-xs text-sidebar-dim">
                Thêm mới hoặc cập nhật hàng loạt nhân khẩu & căn hộ trực thuộc
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="bg-surface-alt rounded-xl p-4 border border-brand-border flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="text-xs font-bold text-ink uppercase tracking-wider flex items-center gap-2">
                <span className="w-5 h-5 rounded-full bg-accent text-white flex items-center justify-center text-[10px] font-mono">1</span>
                <span>File Mẫu Excel Chuẩn Hóa</span>
              </div>
              <p className="text-xs text-ink-soft leading-relaxed">
                Tải file mẫu Excel chuẩn để điền thông tin Họ tên, CCCD, SĐT, Mã căn hộ, Quan hệ, v.v.
              </p>
            </div>
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg bg-accent hover:bg-accent-hover text-white font-semibold text-xs shadow-sm transition-colors duration-200 flex items-center gap-2 shrink-0 cursor-pointer focus-visible:ring-2 focus-visible:ring-accent/40"
            >
          ...
          <div className="space-y-2.5">
            <div className="text-xs font-bold text-ink uppercase tracking-wider flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-accent text-white flex items-center justify-center text-[10px] font-mono">2</span>
            ...
            {!file ? (
              <label
                htmlFor="resident-excel-file-input"
                className="flex flex-col items-center justify-center w-full min-h-[150px] p-6 border-2 border-dashed border-brand-border rounded-xl bg-surface-alt/70 hover:bg-surface-alt hover:border-accent transition-colors duration-200 cursor-pointer text-center group"
              >
                <div className="w-12 h-12 rounded-xl bg-accent-soft text-accent-ink flex items-center justify-center mb-3 group-hover:bg-accent group-hover:text-white transition-colors duration-200">
          ...
          <div className="bg-surface-alt rounded-xl p-4 border border-brand-border space-y-3">
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-accent rounded border-brand-border focus:ring-accent cursor-pointer"
              />
              <span className="text-xs text-ink font-medium">
                Tự động cấp tài khoản đăng nhập Cổng Cư Dân (Mật khẩu mặc định: <code className="text-accent font-mono">123456</code>)
              </span>
            </label>
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              />
              <span className="text-xs text-ink font-medium">
                Cập nhật thông tin nếu cư dân đã tồn tại (khớp theo Số CCCD/CMND hoặc Số điện thoại)
              </span>
            </label>
          </div>
                ...
                <div className="mt-2 text-xs max-h-28 overflow-y-auto bg-surface p-2.5 rounded-lg border border-brand-border font-mono space-y-1">
                  {result.errors.map((e, idx) => (
                    <div key={idx} className="text-brand-danger flex items-start gap-1">
                      <span>•</span>
                      <span>Dòng {e.row} {e.name ? `(${e.name})` : ''}: {e.error}</span>
                    </div>
                  ))}
                </div>
              )}
